...
export default function LoginForm() {
  ...
  async function signIn(formObject) {
    try {
      const user = await Auth.signIn(formObject.email, formObject.password);
      if (user) {
        navigate("/currentReservations", { replace: true });
        console.log("user => ", user);
        sessionStorage.setItem("name", user.attributes.name);
        sessionStorage.setItem("email", user.attributes.email);
        // sessionStorage.setItem("token", res.data.token);
        sessionStorage.setItem("userDocID", user.attributes.sub);
